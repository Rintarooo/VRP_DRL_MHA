...
class Env():
	...
	def _get_step(self, next_node, D):
		...
		# visited_mask = one_hot.torch.bool().transpose(0,2,1)
		# visited_mask = (one_hot == 1).transpose(0,2,1)
		...
		# prev_node_embedding = torch.gather(input = self.node_embeddings, dim = 1, index = next_node)
	...
	def create_context_D_t1(self):
		...
		# print((depot_idx[:,:,None].repeat(1,1,self.embed_dim) > 1).any())
		# depot_embedding = torch.gather(input = self.node_embeddings, dim = 1, index = depot_idx)
