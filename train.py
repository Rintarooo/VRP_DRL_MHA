...
def train(cfg, log_path = None):
	...
	for epoch in tqdm(range(cfg.epochs), desc = 'epoch'):
		...
		for t, inputs in tqdm(enumerate(dataset.batch(cfg.batch))):
			...
			if t%100 == 0:
				print('epoch%d, after %d/%dsamples: loss %1.2f, average L %1.2f, baseline average M %1.2f\n'%(
						epoch, t*cfg.batch, cfg.n_samples, ave_loss.result().numpy(), ave_L.result().numpy(), baseline.M))
		...
		if cfg.islogger:
				...
				with open(log_path, 'a') as f:
					t2 = time()
					f.write('%dmin%dsec,%d,%1.2f,%1.2f\n'%((t2-t1)//60, (t2-t1)%60, epoch, loss_list[-1], L_list[-1]))
